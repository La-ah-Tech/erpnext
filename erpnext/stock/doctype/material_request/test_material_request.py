...
class TestMaterialRequest(FrappeTestCase):
    ...
    def test_completed_qty_for_purchase(self):
        ...
        self.assertEqual(
            current_requested_qty_item1, existing_requested_qty_item1 + 27.0
        )
        self.assertEqual(
            current_requested_qty_item2, existing_requested_qty_item2 + 1.5
        )
        ...
        self.assertEqual(
            current_requested_qty_item1, existing_requested_qty_item1 + 54.0
        )
        self.assertEqual(
            current_requested_qty_item2, existing_requested_qty_item2 + 3.0
        )
    ...
    def _get_requested_qty(self, item_code, warehouse):
        return flt(
            frappe.db.get_value(
                "Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty"
            )
        )
    ...
    def test_requested_qty_multi_uom(self):
        existing_requested_qty = self._get_requested_qty(
            "_Test FG Item", "_Test Warehouse - _TC"
        )
        ...
        requested_qty = self._get_requested_qty(
            "_Test FG Item", "_Test Warehouse - _TC"
        )
    ...
    def test_multi_uom_for_purchase(self):
        ...
        if not frappe.db.get_value(
            "UOM Conversion Detail", {"parent": item.item_code, "uom": "Kg"}
        ):
    ...
    def test_customer_provided_parts_mr(self):
        ...
        existing_requested_qty = self._get_requested_qty(
            "_Test Customer", "_Test Warehouse - _TC"
        )

        mr = make_material_request(
            item_code="CUST-0987", material_request_type="Customer Provided"
        )
        se = make_stock_entry(mr.name)
        se.insert()
        se.submit()
        ...
        mr = frappe.get_doc("Material Request", mr.name)
        mr.submit()
        current_requested_qty = self._get_requested_qty(
            "_Test Customer", "_Test Warehouse - _TC"
        )
