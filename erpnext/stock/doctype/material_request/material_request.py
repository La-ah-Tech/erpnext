...
class MaterialRequest(BuyingController):
    ...
    if TYPE_CHECKING:
        ...
        from erpnext.stock.doctype.material_request_item.material_request_item import (
            MaterialRequestItem,
        )
    ...
    def validate_qty_against_so(self):
        ...
        for so_no in so_items.keys():
            for item in so_items[so_no].keys():
                ...
                if actual_so_qty and (
                    flt(so_items[so_no][item]) + already_indented > actual_so_qty
                ):
                    frappe.throw(
                        _(
    ...
    def set_title(self):
        ...
        if not self.title:
            ...
            self.title = _("{0} Request for {1}").format(
                _(self.material_request_type), items
            )[:100]
    ...
    def check_modified_date(self):
        ...
        date_diff = frappe.db.sql(
            f"""select TIMEDIFF('{mod_db[0][0]}', '{cstr(self.modified)}')"""
        )
    ...
    def get_mr_items_ordered_qty(self, mr_items):
        ...
        if self.material_request_type in (
            "Material Issue",
            "Material Transfer",
            "Customer Provided",
        ):
    ...
    def update_completed_qty(self, mr_items=None, update_modified=True):
        ...
        mr_qty_allowance = frappe.db.get_single_value(
            "Stock Settings", "mr_qty_allowance"
        )
        ...
        for d in self.get("items"):
            ...
            if d.name in mr_items:
                if self.material_request_type in (
                    ...
                    if mr_qty_allowance:
                        allowed_qty = flt(
                            (d.qty + (d.qty * (mr_qty_allowance / 100))),
                            d.precision("ordered_qty"),
                        )

                        if d.ordered_qty and flt(d.ordered_qty, precision) > flt(
                            allowed_qty, precision
                        ):
                            frappe.throw(
                                _(
                                    "The total Issue / Transfer quantity {0} in Material Request {1}  cannot be greater than allowed requested quantity {2} for Item {3}"
                                ).format(
                                    d.ordered_qty, d.parent, allowed_qty, d.item_code
                                )
                            )

                    elif d.ordered_qty and flt(d.ordered_qty, precision) > flt(
                        d.stock_qty, precision
                    ):
                        frappe.throw(
                            _(
        ...
        for mr, mr_item_rows in material_request_map.items():
            if mr and mr_item_rows:
                ...
                if mr_obj.status in ["Stopped", "Cancelled"]:
                    frappe.throw(
                        _("{0} {1} is cancelled or stopped").format(
                            _("Material Request"), mr
                        ),
                        frappe.InvalidStatusError,
                    )
...
def get_items_based_on_default_supplier(supplier):
    supplier_items = [
        ...
        for d in frappe.db.get_all(
            "Item Default",
            {"default_supplier": supplier, "parenttype": "Item"},
            "parent",
        )
    ]
...
def get_material_requests_based_on_supplier(
    ...
    if not supplier_items:
        frappe.throw(
            _("{0} is not the default supplier for any items.").format(supplier)
        )
...
def raise_work_orders(material_request):
    ...
    if errors:
        frappe.throw(
            _("Work Order cannot be created for following reason: <br> {0}").format(
                new_line_sep(errors)
            )
        )
