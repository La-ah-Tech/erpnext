...
# LaahTech-EM - web based ERP (http://erpnext.com)
...
class TestStockReconciliation(FrappeTestCase, StockTestMixin):
    ...
    def _test_reco_sle_gle(self, valuation_method):
        item_code = self.make_item(
            properties={"valuation_method": valuation_method}
        ).name

        se1, se2, se3 = insert_existing_sle(
            warehouse="Stores - TCP1", item_code=item_code
        )
        ...
        for d in input_data:
            ...
            qty_after_transaction = (
                flt(d[0]) if d[0] != "" else flt(last_sle.get("qty_after_transaction"))
            )

            valuation_rate = (
                flt(d[1]) if d[1] != "" else flt(last_sle.get("valuation_rate"))
            )
            ...
            if qty_after_transaction == last_sle.get(
                ...
                self.assertEqual(
                    flt(sle[0].qty_after_transaction, 1), flt(qty_after_transaction, 1)
                )
                self.assertEqual(
    ...
    def test_get_items(self):
        ...
        items = get_items(
            "_Test Warehouse Group 1 - _TC", nowdate(), nowtime(), "_Test Company"
        )
    ...
    def test_stock_reco_for_serial_and_batch_item(self):
        ...
        self.assertEqual(
            frappe.db.get_value("Serial No", serial_nos[0], "batch_no"), batch_no
        )

        sr.cancel()

        self.assertEqual(
            frappe.db.get_value("Serial No", serial_nos[0], "warehouse"), None
        )
    ...
    def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
        ...
        serial_no_2 = get_serial_nos_from_bundle(
        ...
        self.assertEqual(
            frappe.db.get_value("Serial No", reco_serial_no, "batch_no"), batch_no
        )
    ...
    def test_backdated_stock_reco_future_negative_stock(self):
        ...
        pr1_balance = frappe.db.get_value(
            "Stock Ledger Entry",
            {"voucher_no": pr1.name, "is_cancelled": 0},
            "qty_after_transaction",
        )
    ...
    def test_backdated_stock_reco_cancellation_future_negative_stock(self):
        ...
        repost_exists = bool(
            frappe.db.exists(
                "Repost Item Valuation", {"voucher_no": sr.name, "status": "Queued"}
            )
        )
        self.assertFalse(
            repost_exists,
    ...
    def test_serial_no_cancellation(self):
        ...
        se1 = make_stock_entry(
            item_code=item_code, target=warehouse, qty=10, basic_rate=700
        )
    ...
    def test_backdated_stock_reco_entry(self):
        ...
        self.assertFalse(
            frappe.db.exists("Repost Item Valuation", {"voucher_no": stock_reco.name})
        )
    ...
    def test_not_reconcile_all_batch(self):
        ...
        for row in frappe.get_all(
            "Repost Item Valuation", filters={"voucher_no": sr.name}
        ):
    ...
    def test_skip_reposting_for_entries_after_stock_reco(self):
        ...
        item_code = create_item(
            "Test Item For Skip Reposting After Stock Reco", is_stock_item=1
        ).name
    ...
    def test_stock_reco_for_negative_batch(self):
        ...
        sabb_doc = frappe.get_doc(
            "Serial and Batch Bundle", doc.serial_and_batch_bundle
        )
        for row in sabb_doc.entries:
            row.db_set("qty", -20)

        batch_qty = get_batch_qty(
            batch_no, warehouse, item_code, consider_negative_batches=True
        )
...
def create_stock_reconciliation(**args):
    ...
    sr.expense_account = args.expense_account or (
        (
            ...
            or frappe.get_cached_value(
                "Account",
                {"account_type": "Stock Adjustment", "company": sr.company},
                "name",
            )
        )
    ...
    sr.cost_center = (
        ...
        or frappe.get_cached_value(
            "Cost Center", filters={"is_group": 0, "company": sr.company}
        )
    )

    bundle_id = None
    if (
        not args.use_serial_batch_fields
        and (args.batch_no or args.serial_no)
        and args.qty
    ):
