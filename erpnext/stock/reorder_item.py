...
def reorder_item():
    ...
    if not (
        frappe.db.a_row_exists("Company") and frappe.db.a_row_exists("Fiscal Year")
    ):
        return
...
def get_item_warehouse_projected_qty(items_to_consider):
    ...
    for item_code, warehouse, projected_qty in frappe.db.sql(
        ...
        while warehouse_doc.parent_warehouse:
            if not item_warehouse_projected_qty.get(item_code, {}).get(
                warehouse_doc.parent_warehouse
            ):
                item_warehouse_projected_qty.setdefault(item_code, {})[
                    warehouse_doc.parent_warehouse
                ] = flt(projected_qty)
                ...
                for d in items:
                    ...
                    mr.append(
                        "items",
                        {
                            ...
                            "schedule_date": add_days(
                                nowdate(), cint(item.lead_time_days)
                            ),
                            "qty": qty,
...
def notify_errors(exceptions_list):
    subject = _("[Important] [LaahTech-EM] Auto Reorder Errors")
    content = (
