...
def execute():
    ...
    reports = frappe.get_all(
        "Report", {"module": "education", "is_standard": "Yes"}, pluck="name"
    )
    ...
    forms = frappe.get_all(
        "Web Form", {"module": "education", "is_standard": 1}, pluck="name"
    )
    for form in forms:
        frappe.delete_doc("Web Form", form, ignore_missing=True, force=True)

    dashboards = frappe.get_all(
        "Dashboard", {"module": "education", "is_standard": 1}, pluck="name"
    )
    ...
    cards = frappe.get_all(
        "Number Card", {"module": "education", "is_standard": 1}, pluck="name"
    )
    for card in cards:
        frappe.delete_doc("Number Card", card, ignore_missing=True, force=True)

    doctypes = frappe.get_all(
        "DocType", {"module": "education", "custom": 0}, pluck="name"
    )
    ...
    items = frappe.get_all(
        "Portal Menu Item", filters=[["title", "in", titles]], pluck="name"
    )
    for item in items:
    ...
    click.secho(
        ...
        "Please install the app to continue using the module: https://github.com/La-ah-Tech/education",
        fg="yellow",
    )
