...
def get_pegged_rate(
    from_currency: str, to_currency: str, transaction_date
) -> float | None:
    ...
    try:
        ...
        if not value:
            ...
            response = requests.get(
                format_ces_api(settings.api_endpoint, req_params), params=params
            )
            # expire in 6 hours
...
def _enable_all_roles_for_admin():
    ...
    admin_roles = set(
        frappe.db.get_values(
            "Has Role", {"parent": "Administrator"}, fieldname="role", pluck="role"
        )
    )
...
def welcome_email():
    site_name = get_default_company() or "LaahTech-EM"
