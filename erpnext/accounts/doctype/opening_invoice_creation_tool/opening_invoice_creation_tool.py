...
class OpeningInvoiceCreationTool(Document):
    ...
    def onload(self):
        ...
        self.set_onload(
            "temporary_opening_account", get_temporary_opening_account(self.company)
        )
    ...
    def get_opening_invoice_summary(self):
        def prepare_invoice_summary(doctype, invoices):
            ...
            for invoice in invoices:
                ...
                _summary.update(
                    {"currency": company_wise_currency.get(company), doctype: invoice}
                )
                ...
                max_count.update(
                    {
                        doctype: {
                            ...
                            "max_due": (
                                max(outstanding_amount) if outstanding_amount else 0.0
                            ),
                        }
                    }
                )
        ...
        companies = frappe.get_all(
            "Company", fields=["name as company", "default_currency as currency"]
        )
        if not companies:
            return None, None
        ...
        for doctype in ["Sales Invoice", "Purchase Invoice"]:
            invoices = frappe.get_all(
                doctype,
                filters=dict(is_opening="Yes", docstatus=1),
                fields=fields,
                group_by="company",
            )
    ...
    def set_missing_values(self, row):
        ...
        row.temporary_opening_account = (
            row.temporary_opening_account or get_temporary_opening_account(self.company)
        )
        ...
        for row in self.invoices:
            ...
            company_details = (
                frappe.get_cached_value(
                    "Company",
                    self.company,
                    ["default_currency", "default_letter_head"],
                    as_dict=1,
                )
                or {}
            )
            ...
            default_currency = frappe.db.get_value(
                row.party_type, row.party, "default_currency"
            )
                ...
                invoice.update(
                    {
                        ...
                        or company_details.get("default_currency"),
    ...
    def get_invoice_dict(self, row=None):
        def get_item_dict():
            ...
            if not cost_center:
                frappe.throw(
                    _("Please set the Default Cost Center in {0} company.").format(
        ...
        invoice = frappe._dict(
            {
                ...
                "update_stock": 0,  # important: https://github.com/La-ah-Tech/erpnext/pull/23559
    ...
    if errors:
        frappe.msgprint(
            _(
                ...
                errors,
                "<a href='/app/List/Error Log' class='variant-click'>Error Log</a>",
            ),
            indicator="red",
...
def get_temporary_opening_account(company=None):
    ...
    accounts = frappe.get_all(
        "Account", filters={"company": company, "account_type": "Temporary"}
    )
    if not accounts:
