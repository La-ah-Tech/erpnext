...
class TestBankTransaction(FrappeTestCase):
    def setUp(self):
        ...
        bank_account = create_bank_account(
            ...
            bank_account_name="Checking Account " + uniq_identifier,
        )

        add_transactions(bank_account=bank_account)
        add_vouchers(gl_account=gl_account)

    # This test checks if LaahTech-EM is able to provide a linked payment for a bank transaction based on the amount of the bank transaction.
    ...
    def test_reconcile(self):
        ...
        clearance_date = frappe.db.get_value(
            "Payment Entry", payment.name, "clearance_date"
        )
    ...
    # Check if LaahTech-EM can correctly filter a linked payments based on the debit/credit amount
    ...
    def test_clear_sales_invoice(self):
        ...
        payment = frappe.get_doc(
            "Sales Invoice", dict(customer="Fayva", status=["=", "Paid"])
        )
        vouchers = json.dumps(
            [
                {
        ...
        self.assertTrue(
            frappe.db.get_value(
                "Sales Invoice Payment", dict(parent=payment.name), "clearance_date"
            )
            is not None
        )
    ...
    def test_matching_loan_repayment(self):
        ...
        linked_payments = get_linked_payments(
            bank_transaction.name, ["loan_repayment", "exact_match"]
        )
...
def add_vouchers(gl_account="_Test Bank - _TC"):
    ...
    pi = make_purchase_invoice(
        supplier="Poore Simon's", qty=1, rate=3900, is_paid=1, do_not_save=1
    )
    ...
    if not frappe.db.get_value(
        ...
        mode_of_payment.append(
            "accounts", {"company": "_Test Company", "default_account": gl_account}
        )
    ...
    si.is_pos = 1
    si.append(
        "payments", {"mode_of_payment": "Cash", "account": gl_account, "amount": 109080}
    )
    si.insert()
    si.submit()
...
def create_loan_and_repayment():
    ...
    loan = create_loan(
        applicant, "Personal Loan", 5000, "Repay Over Number of Periods", 20
    )
    loan = frappe.get_doc(
        {
            "doctype": "Loan",
    ...
    make_loan_disbursement_entry(
        loan.name, loan.loan_amount, disbursement_date="2018-09-27"
    )
