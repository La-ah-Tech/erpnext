...
class PricingRule(Document):
    ...
    if TYPE_CHECKING:
        ...
        from erpnext.accounts.doctype.pricing_rule_brand.pricing_rule_brand import (
            PricingRuleBrand,
        )
    ...
    def validate_duplicate_apply_on(self):
        if self.apply_on != "Transaction":
            ...
            values = [
                d.get(apply_on_field)
                for d in self.get(apply_on_table)
                if d.get(apply_on_field)
            ]
            if len(values) != len(set(values)):
                frappe.throw(
                    _("Duplicate {0} found in the table").format(self.apply_on)
                )
    ...
    def validate_mandatory(self):
        ...
        if tocheck and not self.get(tocheck):
            throw(
                _("{0} is required").format(self.meta.get_label(tocheck)),
                frappe.MandatoryError,
            )
    ...
    def cleanup_fields_value(self):
        ...
        cleanup_other_fields = (
            ...
            if not apply_rule_on_other
                ...
                if o_field != "other_" + apply_rule_on_other
            ]
        )
    ...
    def validate_max_discount(self):
        if self.rate_or_discount == "Discount Percentage" and self.get("items"):
            for d in self.items:
                max_discount = frappe.get_cached_value(
                    "Item", d.item_code, "max_discount"
                )
                if max_discount and flt(self.discount_percentage) > flt(max_discount):
                    throw(
                        _("Max discount allowed for item: {0} is {1}%").format(
                            d.item_code, max_discount
                        )
                    )
    ...
    def validate_price_list_with_currency(self):
        if self.currency and self.for_price_list:
            price_list_currency = frappe.db.get_value(
                "Price List", self.for_price_list, "currency", True
            )
            if not self.currency == price_list_currency:
                throw(
                    _("Currency should be same as Price List Currency: {0}").format(
                        price_list_currency
                    )
                )
    ...
    def validate_dates(self):
        if self.is_cumulative and not (self.valid_from and self.valid_upto):
            frappe.throw(
                _("Valid from and valid upto fields are mandatory for the cumulative")
            )
...
def update_args_for_pricing_rule(args):
    if not (args.item_group and args.brand):
        ...
        if not args.item_group:
            frappe.throw(
                _("Item Group not mentioned in item master for item {0}").format(
                    args.item_code
                )
            )
    ...
    if args.transaction_type == "selling":
        ...
        args.supplier_group = frappe.get_cached_value(
            "Supplier", args.supplier, "supplier_group"
        )
...
def apply_price_discount_rule(pricing_rule, item_details, args):
    ...
    if pricing_rule.rate_or_discount == "Rate":
        ...
        # TODO https://github.com/La-ah-Tech/erpnext/pull/23636 solve this in some other way.
...
def get_item_uoms(doctype, txt, searchfield, start, page_len, filters):
    ...
    if filters.get("apply_on") != "Item Code":
        ...
        items = [
            d.name
            for d in frappe.db.get_all("Item", filters={field: filters.get("value")})
        ]
