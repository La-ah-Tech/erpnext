...
def validate_webhooks_request(doctype, hmac_key, secret_key="secret"):
    def innerfn(fn):
        ...
        if (
            ...
            if frappe.request.data and not sig == bytes(
                frappe.get_request_header(hmac_key).encode()
            ):
...
def get_webhook_address(connector_name, method, exclude_uri=False, force_https=False):
    endpoint = f"erpnext.laahtech_em_integrations.connectors.{connector_name}.{method}"
...
def get_tracking_url(carrier, tracking_number):
    ...
    if url_reference:
        tracking_url = frappe.render_template(
            url_reference, {"tracking_number": tracking_number}
        )
    return tracking_url
